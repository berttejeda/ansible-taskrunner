  ...
  def init_cli_sub_command(self, cli_obj, cli, **kwargs):
    ...
    for cmd, opt in commands.items():
        for c in cmd.split('|'):
          ...
          # Add special variable for current subcommand
          cli_obj.available_vars['__command__'] = c
          _help_string = cli_obj.yaml_vars.get(f'commands.{cmd}.help.message', '')
          help_string = Template(_help_string).safe_substitute(**cli_obj.available_vars)
          examples = cli_obj.yaml_vars.get(f'commands.{cmd}.help.examples', '')
          examples_string = ''
          epilog = ''
          ...
          # Shell functions
          shell_functions = []
          internal_functions = cli_obj.yaml_vars.get(f'commands.{cmd}.functions', {})
          ...
          command_vars = cli_obj.yaml_vars.get(f'commands.{cmd}')
