...
class ExtendCLI():
    ...
    def process_options(self, parameters, func, is_required=False):
        ...
        vanilla_parameters = dict([(k,v) for k,v in parameters.items() if not isinstance(parameters[k], dict)])
        ...
        for cli_option, value in parameters.items():
            ...
            if '|' in cli_option:
                ...
                option = click.option(first_option, second_option, value, type=str, nargs=numargs, help=option_help, required=is_required)
            else:
    ...
    def options(self, func):
        ...
        extended_cli_func_required = self.process_options(required_parameters, func, is_required=True)
        optional_parameters = self.vars.get('optional_parameters', {})
        extended_cli_func = self.process_options(optional_parameters, extended_cli_func_required)
